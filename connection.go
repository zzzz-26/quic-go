...
import (
	...
	"github.com/zzzz-26/quic-go/internal/flowcontrol"
	"github.com/zzzz-26/quic-go/internal/handshake"
	"github.com/zzzz-26/quic-go/internal/logutils"
	...
	if s.logger.Debug() {
		/*
			if len(packet.longHdrPackets) > 1 {
				...
				s.logger.Debugf("-> Sending packet %d (%d bytes) for connection %s, %s", 0, packet.buffer.Len(), s.logID, "")
			}
		*/
	}
	for _, p := range packet.longHdrPackets {
